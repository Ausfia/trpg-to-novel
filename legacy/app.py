...
def _tab_pipeline(camp: Campaign):
    ...
    # 已入章场景统计：基于 story_state.yaml
    state_yaml = _load_state(str(camp.root))
    ...
    for sid in sessions_with_scenes:
        scenes = _load_scenes(str(camp.parsed_dir), sid)
        total_scenes += len(scenes)
        for sc in scenes:
            if sc.get("id") not in processed_ids:
                pending_scenes += 1
    st.caption(
        f"📚 本团已生成 **{len(chapter_index)}** 章 · "
        f"已入章场景 **{len(processed_ids)}** / 总场景 **{total_scenes}** · "
        f"待处理 **{pending_scenes}**"
    )
    ...
    if not _stage_value(env, "draft", "api_key").strip():
        ...
        st.info("所有场景均已入章。如需重新生成，请在「审稿」标签删除对应章节，或手动编辑 story_state.yaml。")
        draft_disabled = True
    else:
...
def _tab_review(camp: Campaign):
    ...
    # 章节元信息（从 story_state.chapter_index 读，若有）
    if chapters and sel_chap is not None:
        _state_yaml = _load_state(str(camp.root))
        _index = _state_yaml.get("chapter_index") or []
